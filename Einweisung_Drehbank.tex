...
% unserermeinung nach fertig. (Toni, Philipp)
...
% keine Überschneidung, Thematisch hier passend.
...
Bei Unfall: Rettungsdienst rufen (Tel. 09-112 / Handy: 112), Verletzten betreuen, ins Verbandbuch eintragen.
	...
	\item \enquote{Gewaltwerkzeug} wie Hammer oder Zange kann irreparablen Schaden anrichten und hat deshalb in der Drehbank nichts verloren. In bestimmten Fällen kann ein Schonhammer eingesetzt werden. % Diese Zeile NICHT entfernen, ohne vorher Rücksprache mit Max aufzunehmen! lol.
	\item Stangenmaterial darf nicht aus der Maschine ragen, da keine Schutzabdeckung vorhanden ist. % Wir bräuchten eine stabile Fangeinrichtung
	\item Drehen zwischen Spitzen ist nicht zulässig. % Wir haben kein Sicherheitsdrehherz o.Ä.
\end{itemize}
...
% kommt etwas weiter unten bei "Schutzausrüstung und Gefahren" nochmal.
%\item \textbf{Bei Handbetrieb im Sicherheitsbereich immer Schutzbrille oder Gesichtsschild zu tragen!}
...
\begin{itemize}
\item Späne nur bei stehender Maschine mit geeignetem Werkzeug (z.B. Pinsel, Staubsauger; nicht: Druckluft, Hand) entfernen. % per Hand nicht zulässig nach BGI 5003 2.5., selbst für Alu??? -> leider ja. 
	\item Kontakt mit Kühlschmierstoff vermeiden, Hände waschen.
	\item \todo{Hautschutz: Handcreme bereitstellen mit Spender und darauf verweisen?}
...
% CNC Rändeln nicht vorhanden, auch keine Ansätze dazu vorhanden.
...
%\todoUnwichtig{Installation erklären}
% ist egal. dazu gibt es das Softwarehandbuch. Im Lab sind diese Dateien eingerichtet und für die Benutzung vorbereitet.
...
% todo entfernt, weil diese Aussage so richtig ist.
...
\subsubsection{Werkzeuge}
Die Werkzeugbibliothek von NX enthält die im FabLab vorhandenen Werkzeuge mit bereits richtig voreingestellten Eigenschaften und Abmessungen. \todoUnwichtig{Auffinden dieser Werkzeuge? Bezeichnungsschema?}

\todoUnwichtig{Laden der Werkzeugbibliothek?}

\todoUnwichtig{gespiegelte Anzeige oder nicht? bisher widersprüchliche Informationen und verschiedene Ansätze (arw/Toni/Max), höchstwahrscheinlich sogar inkonsistent zu Maxs Online-Kurzanleitung}
...
\todoUnwichtig{Avoidance/Containment, Anfahrbewegungen hier erklären}

\subsubsection{Postprocessing}

Die Endgültige Erzeugung von GCode erfolgt im "`Postprocessing"' genannten Arbeitsschritt. Hierzu wird ein für die im FabLab verwendete Maschine erstelltes Postprocessing-Programm verwendet. \todoUnwichtig{Beschreibung Auswahl postprocessing}
