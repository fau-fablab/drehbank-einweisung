...
\tikzstyle{abschnitt}=[text width=19cm,draw,black,fill=white];%
\hspace{-2.5cm}
\begin{tikzpicture}[node distance=1em]
\node [fill=blue,rectangle,minimum width=21.5cm,minimum height=29.7cm,anchor=north east] (seite) at (0,0)  {};
\newcommand{\abschnittTitel}[1]{\vspace{-2em}\subsection{#1}}

\node[abschnitt,below=1.5cm of seite.north] (anwendungsbereich) {{\vspace{-2em}\section{Betriebsanweisung Drehmaschine Wabeco CC-D6000hs im FAU FabLab} \label{sec:betriebsanweisung}}
% 	\abschnittTitel{Anwendungsbereich}
 ...
 \node[abschnitt, below=of anwendungsbereich] (gefahren) {
	\abschnittTitel{Gefahren für Mensch und Umwelt}
	\begin{itemize}
		\item Erfasstwerden der Haare, Kleidung, Schmuck usw. durch Antrieb, Spindel, Werkzeug oder Werkstück.
		...
		\item Intensiver Hautkontakt mit Kühlschmierstoff führt zur Zerstörung des Säureschutzmantels, Entfettung, 
		\item Entwässerung und Reizung der Haut als Vorstufe von Hauterkrankungen.
		\item Schon geringfügige Hautverletzungen, z.B. durch Metallteilchen, erhöhen das Risiko einer kühlschmierstoffbedingten Hauterkrankung.
	\end{itemize}
};

\node[abschnitt, below=of gefahren] (vorschriften) {
\abschnittTitel{Schutzmaßnahmen und Verhaltensregeln}
Generell:
\begin{itemize}
...
Im Handbetrieb:
\begin{itemize}
 \item Lange Haare durch Mütze, Haarnetz o.\,Ä. verdecken. Zusammenbinden mit Zopfgummi ist nicht ausreichend!
...
\node[abschnitt, text width=9cm, below right=1em and 0em of vorschriften.south west] (unfall) {
	\abschnittTitel{Verhalten bei Unfällen}
	\begin{itemize}
	\item Maschine abschalten. - NOT-AUS drücken
	\item Betreuer informieren. Gegebenenfalls Rettungsdienst rufen (Tel. 09-112 / Handy: 112)
...
\node[abschnitt, text width=9cm, below right= 0em and 0.5cm of unfall.north east] (stoerung) {
	...
	\begin{itemize}
	\item Ausschalten und Betreuer informieren. Schadensmeldung sichtbar an der Maschine anbringen.
...
\node[abschnitt, below right= 1em and 0em of unfall.south west] (instandhaltung) {
	...
	\item Für die Instandhaltung ist die Zerspanungsgruppe (zerspanung@fablab.fau.de) zuständig.
	\end{itemize}
};
\node[abschnitt, text width=9em, below left = 1em and 0em of instandhaltung.south east] (datum) {Stand: \today};
\end{tikzpicture}
...
\todo{ generell die folgenden Abschnitte zu NX eventuell erweitern }
...
\begin{itemize}
	...
	\item Werkzeug 3 einsetzen\footnote{Es ist auch möglich ein anderes Werkzeug als Werkzeug 3 zum Setzen des Nullpunktes zu verwenden. Hierzu müssen aber zuerst die Werkzeugoffsets dieses anderen Werkzeugs geladen werden. Hierzu muss man manuell in der Registerkarte \encode{MDI (F5)} den GCode \verb#M6 Tn# wobei $n$ die Werkzeugnummer ist, also beispielsweise \verb#M6 T2# für Werkzeug Nummer 2 eingeben. Anschliessend muss noch \verb#G43# eingegeben werden um das Koordinatensystem dem grade gesetzten Werkzeug anzupassen. Nach dem Rückwechsel auf die Registerkarte \encode{Manuelle Kontrolle (F3)} kann man in der Anleitung normal weiter fortfahren. Es ist zu beachten, dass sich nicht alle Werkzeuge aufgrund ihrer Geometrie zur Nullpunktseinstellung eignen.}
